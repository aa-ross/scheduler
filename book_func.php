...
function view_cat ($dbc){
	...
	if($num>0){
	 ...
	 //	echo "<option value=$id2>$nm</option>";
	 		echo "<span id='$id' class='button'>$nm</span> | ";
		}
		return $i;
...
function view_serv ($dbc, $id){
	...
	if($num>0){
	 	...
	 	while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
	 		$nm = $row['serv_title'];
	 		$id2 = $row['serv_id'];
	 		echo "<span id='se$id2' class='button'>$nm</span> | ";
	 		//array_push($a,$id2);
	 		//return $a;
		}
...
function view_dates($dbc, $month, $year){
	...
	while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
		...
		echo "<li id='$month$day'><a href='#'>$day</a></li>";
			if($i=='7'){
				$i=0;
			} else {
				$i++;
	...
	while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)){
		...
		while($i>0){
			...
			if($avail=='no'){
				...
				if($i=='1'){
					echo "<span id='$id' class='button'>$start</span>";
...
function show_timeslot($dbc, $avail, $serv){
	...
	echo "<span id='$avail-show' class='showing'><br>This appointment is with $emp.<br><p class='booking'><a href='#' id='test'>Book?</a></p></span>";
	echo "<span id='$avail-confirm' class='confirm'>";
	confirm($dbc, $serv, $id, $emp, $ts, $date);
	echo '</span>';
}
...
function confirm($dbc, $serv, $empid, $empname, $time, $date){
	...
	$month = $row['m'];
	$day = $row['d'];
	echo "<h4>You are scheduling an appointment for a(n) $servtitle with $empname at $starttime on $month $day.</h4><br><a href='book3.php?serv=$serv&emp=$empid&time=$time&date=$date'>Confirm</a>";
}
