...
<?php
	$q = "SELECT serv_title, serv_id FROM service ORDER BY serv_title ASC";
	$r = @mysqli_query($dbc, $q);
		if ($r){
			...
			if($num==0){
				...
				while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
	 				$s = $row['serv_title'];
	 				$id = $row['serv_id'];
	 				echo '<a href="delete_service.php?id=' . $id . '"onclick="return confirm(' . "'Are you sure you want to delete " . "$s?'" . ');">' . "<span class='button'>$s</span>" . '</a>';
